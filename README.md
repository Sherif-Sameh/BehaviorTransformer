...
The model receives a sequence of observations $\left(\mathbf{o}\_{t}, \mathbf{o}\_{t-1}, \cdots, \mathbf{o}\_{t-h+1}\right)$ and predicts a corresponding sequence of actions $\left( \mathbf{a}\_{t}, \mathbf{a}\_{t-1}, \cdots, \mathbf{a}\_{t-h+1} \right)$.
...
The focal loss is a modification to the Cross-Entropy Loss such that $\mathcal{L}\_{focal} = -(1 - p\_{t})^\gamma \log(p\_{t})$, where $p\_{t}$ is the predicted probability of the ground truth class. The second term in the combined loss function, named as the multi-task loss $\mathcal{L}\_{multi-task}$, handles the fitting of the continuous offsets of the model.
